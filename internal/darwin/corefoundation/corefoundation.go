...
func NewArray(src []TypeRef) *ArrayRef {
	var values []unsafe.Pointer
	for _, value := range src {
		values = append(values, unsafe.Pointer(value.TypeRef()))
	}
	...
	return &ArrayRef{
		Value: C.CFArrayCreate(AllocatorDefault, ptr, C.CFIndex(numValues), &C.kCFTypeArrayCallBacks),
	}
}
